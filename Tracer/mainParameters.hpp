...
public:
  ...
  enum class GridLocation { BEFORE, AFTER };
  ...
  tParameters()
      : random_engine(static_cast<unsigned long>(
            std::chrono::system_clock::now().time_since_epoch().count())),
        reflection_distribution(0.0, 1.0) {}

  explicit tParameters(char const *initFileName)
      : random_engine(static_cast<unsigned long>(
            std::chrono::system_clock::now().time_since_epoch().count())),
        reflection_distribution(0.0, 1.0) {
  ...
  double set_working_wave(double wave) {
    if (m_reflection_curves.empty()) throw std::logic_error("The curve set is empty.");

    auto nearest =
        std::min_element(m_reflection_curves.begin(), m_reflection_curves.end(),
                         [wave](auto const &curve_a, auto const &curve_b) {
    ...
    return m_reflection_curves[m_working_curve].first;
  }

  TransitivityMap const &transitivity_map() const { return gridMap; }
};
